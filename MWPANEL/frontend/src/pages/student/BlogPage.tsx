...
const StudentBlogPage: React.FC = () => {
    ...
    try {
      ...
      const response = await apiClient.get('/blog-posts', {
        params: {
          ...
          includeStats: true,
          status: 'published',
          // Solo posts visibles para estudiantes
          visibility: ['public', 'students_only', 'class_specific'],
        },
      });
  ...
  const handleViewPost = async (post: BlogPost) => {
    try {
      ...
      await apiClient.post(`/blog-posts/${post.id}/view`);
      ...
      // Mostrar el post aunque falle el registro de vista
      setSelectedPost(post);
      setPostModalVisible(true);
    }
  };
  ...
  const getVisibilityText = (visibility: string) => {
    switch (visibility) {
      ...
      case 'class_specific': return 'Mi Clase';
  ...
  const getAuthorName = (author: any) => {
    ...
    return 'Anónimo'; // RGPD: no mostrar email del autor
  };
  ...
  return (
    <div style={{ padding: '24px' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <Paragraph>
          Descubre las últimas noticias, eventos y recursos educativos de nuestro centro.
        </Paragraph>
      </div>
            ...
            <List.Item
              ...
              actions={[
                <Space key="stats">
                  ...
                  <span><CommentOutlined /> {post.comments?.length || 0}</span>
                </Space>,
                <Button 
                  key="read"
                  type="primary"
                  ...
                  <div>
                    <div style={{ marginBottom: '8px' }}>
                      <Space>
                        ...
                        <Tag>{getVisibilityText(post.visibility)}</Tag>
                      </Space>
                    </div>
                    <Space size="small">
                      <Text type="secondary">
          ...
          <div>
            ...
            <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: '16px' }}>
              <Space>
                ...
                <Text type="secondary">
                  <CommentOutlined /> {selectedPost.comments?.length || 0} comentarios
                </Text>
              </Space>
            </div>
          </div>
