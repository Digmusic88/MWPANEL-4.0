...
const EducationalResourcesPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'my' | 'public' | 'assigned'>('my');
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      switch (activeTab) {
        case 'my':
          // Para profesores, filtrar por su user ID ya que el authorId es el user.id
      ...
      const result = await educationalResourcesService.getResources(queryFilters);
      return result;
    },
  });
  ...
  const formatViewedAt = (dateStr: string) => {
    ...
    if (hoursAgo < 1) {
      ...
      return minutesAgo <= 1 ? 'hace un momento' : `hace ${minutesAgo} minutos`;
    }
    if (hoursAgo < 24) {
      return `hace ${Math.floor(hoursAgo)} hora${Math.floor(hoursAgo) !== 1 ? 's' : ''}`;
    }
        ...
        <Col xs={24} lg={18}>
          {viewMode === 'grid' ? (
            <ResourceGrid
              ...
              onViewersClick={isTeacherOrAdmin ? (id) => {
                const r = data?.resources?.find(x => x.id === id);
                handleViewersClick(id, r?.title || '');
              } : undefined}
              canDelete={activeTab === 'my' && isTeacherOrAdmin}
