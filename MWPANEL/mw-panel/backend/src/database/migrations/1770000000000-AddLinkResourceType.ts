...
export class AddLinkResourceType1770000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. ALTER TYPE ADD VALUE cannot run inside a transaction in PostgreSQL 15.
    //    Commit the current transaction, run it, then open a new one.
    await queryRunner.commitTransaction();
    await queryRunner.query(`
      ALTER TYPE "public"."educational_resources_type_enum" ADD VALUE IF NOT EXISTS 'LINK'
    `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Delete LINK rows before restoring NOT NULL constraints.
    // LINK resources have NULL in Drive columns; SET NOT NULL would fail otherwise.
    await queryRunner.query(`
      DELETE FROM "educational_resources" WHERE type = 'LINK'
    `);
    ...
    // To fully revert the enum, the type must be recreated — only do this in dev.
  }
}
